...
if (QUICKUPDATES_DISPLAY_ID == 'QUICKUPDATES_DISPLAY_ID' && ($_GET['qu_installer'] != 'install')) { // admin area config not installed
	$messageStack->add(sprintf('Quick Updates Configuration Keys Not installed. Please install your configuration by clicking %sInstall%s', '<a class="button" href="' . zen_href_link(FILENAME_QUICK_UPDATES, 'qu_installer=install') . '">', '</a>'), 'warning');
}
...
function zen_quickupdates_table_head($sort_field, $head_text, $cols=1) {
  $str = '';
  $str .= '<td class="dataTableHeadingContent"' . ($cols > 1 ? ' colspan="' . $cols . '"' : '') . '>';
  if($sort_field != '') {
...
?>
      ...
      <table class="quTop">
        <tr>
          ...
          <td class="smallText"><?php echo zen_draw_form('manufacturers', FILENAME_QUICK_UPDATES, zen_get_all_get_params(array('manufacturer')), 'get') . DISPLAY_MANUFACTURERS . '&nbsp;&nbsp' . zen_draw_pull_down_menu("manufacturer", $manufacturers_array, $manufacturer, 'onChange="this.form.submit();"'); ?></form></td>
          <td class="smallText"><?php echo zen_draw_form('manufacturers', FILENAME_QUICK_UPDATES, zen_get_all_get_params(array('manufacturer')), 'get') . DISPLAY_STATUS . '&nbsp;&nbsp' . zen_draw_pull_down_menu("products_status", array('0' => array('id' => 'all', 'text' => 'All'), '1' => array('id' => '1', 'text' => 'Active'), '2' => array('id' => '0', 'text' => 'Inactive')), $_SESSION['quick_updates']['products_status'] ,'onChange="this.form.submit();"'); ?></form></td>
          <td class="smallText"><?php echo zen_draw_form('categorie', FILENAME_QUICK_UPDATES, zen_get_all_get_params(array('cPath')), 'get') . DISPLAY_CATEGORIES . '&nbsp;&nbsp;' . zen_draw_pull_down_menu('cPath', $quick_updates_category_tree, $current_category_id, 'onChange="this.form.submit();"') . '</form>'; ?></td>
          <td>
            <?php
      ...
      <table class="quFormTable"  cellspacing="0">
        <tr>
          <td>
            ...
            <table>
              <tr>
                <td class="smalltext"><?php echo WARNING_MESSAGE; ?> </td>
                <td class="pageHeading">
            ...
            <table id="quickUpdatesProductsTable"  cellspacing="0">
              <tr>
                <td>
                  ...
                  <table class="quickUpdatesTable" cellspacing="0">
...
if(QUICKUPDATES_DISPLAY_EDIT == 'true')
  echo zen_quickupdates_table_head('', '&nbsp;');
    if(QUICKUPDATES_MODIFY_DESCRIPTION == 'false' && QUICKUPDATES_MODIFY_DESCRIPTION_POPUP == 'true')
  echo zen_quickupdates_table_head('', '&nbsp;');
?>
                    </tr>
...
//EOF Product Description Column
    ...
    if(QUICKUPDATES_DISPLAY_TVA_PRICES == 'true'){
      ...
      echo '<td class="smallText productsPriceTax">' . zen_draw_input_field('quick_updates_new[products_taxprice][' . $products->fields['products_id'] . ']', $tax_price, $parameters);
...
<?php
  ...
  echo '<a class="button" title="Print this page"  href="javascript:window.print()">' . PRINT_TEXT . '</a>&nbsp;&nbsp;';
...
?>
        ...
        <tr>
          <td>
            ...
            <table>
              <tr>
                <td class="smallText"><?php echo $products_split->display_count($products_query_numrows, MAX_DISPLAY_ROW_BY_PAGE, $split_page, TEXT_DISPLAY_NUMBER_OF_PRODUCTS);  ?></td>
                <td class="smallText"><?php echo $products_split->display_links($products_query_numrows, MAX_DISPLAY_ROW_BY_PAGE, MAX_DISPLAY_PAGE_LINKS, $split_page); ?></td>
              </tr>
            </table>
